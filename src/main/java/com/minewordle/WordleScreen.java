...
public class WordleScreen extends Screen {
    ...
    private static final String[][] KB_ROWS={{"Q","W","E","R","T","Y","U","I","O","P"},{"A","S","D","F","G","H","J","K","L"},{"ENT","Z","X","C","V","B","N","M","DEL"}};
    ...
    public WordleScreen(boolean practiceMode){
        super(new LiteralText("MineWordle"));this.practiceMode=practiceMode;
        if(practiceMode){game.startPractice();}else if(!WordleSaveManager.load(game)){
            WordleFetcher.fetchTodaysSolution().thenAccept(word->MinecraftClient.getInstance().execute(()->{
    ...
    @Override protected void init(){
        ...
        for(int s=46;s>=12;s--){int kh=s*36/46,kw=s*26/46,totalH=GRID_TOP+6*(s+TILE_GAP)-TILE_GAP+14+3*(kh+KEY_GAP)-KEY_GAP+10;
    ...
    private int tileColor(TileState ts){if(ts==null)return C_KEY_DEFAULT;return switch(ts){case CORRECT->C_CORRECT;case PRESENT->C_PRESENT;case ABSENT->C_ABSENT;default->C_TILE_EMPTY;};}
    private static int darken(int a){int al=(a>>24)&0xFF,r=Math.max(0,((a>>16)&0xFF)-28),g=Math.max(0,((a>>8)&0xFF)-28),b=Math.max(0,(a&0xFF)-28);return(al<<24)|(r<<16)|(g<<8)|b;}
    ...
    private void playWinSound(int g){var s=g<=2?SoundEvents.UI_TOAST_CHALLENGE_COMPLETE:g<=4?SoundEvents.ENTITY_PLAYER_LEVELUP:SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP;MinecraftClient.getInstance().getSoundManager().play(PositionedSoundInstance.master(s,1.0f));}
    private void copyShare(){MinecraftClient.getInstance().keyboard.setClipboard(buildShareText());flash("Copied!");}
    private void sendToChat(){MinecraftClient c=MinecraftClient.getInstance();if(c.player==null){flash("Not in a world!");return;}c.player.sendChatMessage(buildShareText().split("\n")[0]);flash("Sent to chat!");}
    private String buildShareText(){int g=game.getGuessIndex();boolean won=game.getGameState()==GameState.WON;String res=won?g+"/6":"X/6";StringBuilder sb=new StringBuilder();if(practiceMode){sb.append("MineWordle Practice ").append(res).append("\n\n");}else{long d=ChronoUnit.DAYS.between(LocalDate.of(2021,6,20),LocalDate.now(ZoneId.systemDefault()))+1;sb.append("Wordle ").append(d).append(" ").append(res).append("\n\n");}TileState[][] eval=game.getEvaluated();for(int row=0;row<g;row++){for(int col=0;col<5;col++){sb.append(switch(eval[row][col]){case CORRECT->"🟩";case PRESENT->"🟨";default->"⬜";});}if(row<g-1)sb.append("\n");}return sb.toString();}
    ...
    private void renderLoadingStatus(MatrixStack m){if(game.getGameState()==GameState.ERROR)drawCenteredText(m,textRenderer,new LiteralText("Failed to load — check your connection."),this.width/2,GRID_TOP-4,C_RED);}
    private void renderEndOverlay(MatrixStack matrices){
        ...
        int color1=won?C_GREEN:C_RED,boxW=Math.max(Math.max(textRenderer.getWidth(line1),textRenderer.getWidth(line2)),textRenderer.getWidth(copyLabel)+20)+28,boxH=68;
        int boxX=this.width/2-boxW/2,boxY=this.height/2-boxH/2;
        fill(matrices,boxX,boxY,boxX+boxW,boxY+boxH,0xF0101010);drawRect(matrices,boxX,boxY,boxW,boxH,1,0xFF888888);
        ...
        fill(matrices,copyBtnX,copyBtnY,copyBtnX+bw,copyBtnY+bh,0xFF333333);drawRect(matrices,copyBtnX,copyBtnY,bw,bh,1,0xFF666666);
        ...
        fill(matrices,chatBtnX,chatBtnY,chatBtnX+bw,chatBtnY+bh,0xFF333333);drawRect(matrices,chatBtnX,chatBtnY,bw,bh,1,0xFF666666);
    ...
    private void renderGrid(MatrixStack matrices){
        ...
        String cur=game.getCurrentInput();int gi=game.getGuessIndex();boolean pl=game.getGameState()==GameState.PLAYING;
        for(int row=0;row<6;row++)for(int col=0;col<5;col++){
            ...
            else if(row==gi&&pl&&col<cur.length()){letter=String.valueOf(cur.charAt(col));border=C_BORDER_ACTIVE;}
            fill(matrices,x,y,x+TILE_SIZE,y+TILE_SIZE,bg);drawRect(matrices,x,y,TILE_SIZE,TILE_SIZE,2,border);
    ...
    private void renderKeyboard(MatrixStack matrices){
        ...
        for(int row=0;row<KB_ROWS.length;row++){String[] keys=KB_ROWS[row];int rowW=0;
            ...
            for(String key:keys){int kw=isWide(key)?KEY_W_WIDE:KEY_W,bg=C_KEY_DEFAULT;
                ...
                textRenderer.draw(matrices,key,x+kw/2-textRenderer.getWidth(key)/2,y+KEY_H/2-4,C_WHITE);x+=kw+KEY_GAP;}}
    }
    private void renderFlash(MatrixStack m){if(flashTimer<=0)return;flashTimer--;int fw=textRenderer.getWidth(flashMsg),fx=this.width/2-fw/2,fy=GRID_TOP-14;fill(m,fx-6,fy-2,fx+fw+6,fy+12,C_WHITE);textRenderer.draw(m,flashMsg,fx,fy+1,0xFF000000);}
    private boolean isWide(String k){return k.equals("ENT")||k.equals("DEL");}
    private void drawRect(MatrixStack m,int x,int y,int w,int h,int t,int c){fill(m,x,y,x+w,y+t,c);fill(m,x,y+h-t,x+w,y+h,c);fill(m,x,y,x+t,y+h,c);fill(m,x+w-t,y,x+w,y+h,c);}
    private void drawCentered(MatrixStack m,String text,int cx,int cy,int color){textRenderer.draw(m,text,cx-textRenderer.getWidth(text)/2,cy-4,color);}
    ...
    @Override public boolean mouseClicked(double mx,double my,int btn){GameState state=game.getGameState();if((state==GameState.WON||state==GameState.LOST)&&btn==0){if(mx>=copyBtnX&&mx<=copyBtnX+copyBtnW&&my>=copyBtnY&&my<=copyBtnY+copyBtnH){copyShare();return true;}if(mx>=chatBtnX&&mx<=chatBtnX+chatBtnW&&my>=chatBtnY&&my<=chatBtnY+chatBtnH){sendToChat();return true;}}return super.mouseClicked(mx,my,btn);}
