...
public class WordleScreen extends Screen {
    ...
    @Override public void render(int mx,int my,float delta){
        ...
        drawCenteredString(this.textRenderer,"MINEWORDLE",this.width/2,pt+4,C_WHITE);
        fill(this.width/2-80,pt+14,this.width/2+80,pt+15,C_BORDER);
        if(practiceMode)drawCenteredString(this.textRenderer,"- PRACTICE -",this.width/2,pt+17,C_PRACTICE);
        renderLoadingStatus();renderGrid();renderKeyboard();renderEndOverlay();renderFlash();
        super.render(mx,my,delta);
    }
    private void renderLoadingStatus(){if(game.getGameState()==GameState.ERROR)drawCenteredString(this.textRenderer,"Failed to load — check your connection.",this.width/2,GRID_TOP-4,C_RED);}
    private void renderEndOverlay(){
        ...
        drawCenteredString(this.textRenderer,line1,this.width/2,boxY+7,color1);
        drawCenteredString(this.textRenderer,line2,this.width/2,boxY+20,C_WHITE);
        ...
        drawCenteredString(this.textRenderer,copyLabel,this.width/2,copyBtnY+3,C_WHITE);
        ...
        drawCenteredString(this.textRenderer,chatLabel,this.width/2,chatBtnY+3,C_WHITE);
    }
    private void renderGrid(){
        ...
        for(int row=0;row<6;row++)for(int col=0;col<5;col++){
            ...
            fill(x,y,x+TILE_SIZE,y+TILE_SIZE,bg);drawRect(x,y,TILE_SIZE,TILE_SIZE,2,border);
            if(!letter.isEmpty())drawCentered(letter,x+TILE_SIZE/2,y+TILE_SIZE/2,C_WHITE);
        }
    }
    private void renderKeyboard(){
        ...
        for(int row=0;row<KB_ROWS.length;row++){String[] keys=KB_ROWS[row];int rowW=0;
            ...
            for(String key:keys){int kw=isWide(key)?KEY_W_WIDE:KEY_W,bg=C_KEY_DEFAULT;
                ...
                textRenderer.draw(key,x+kw/2-textRenderer.getWidth(key)/2,y+KEY_H/2-4,C_WHITE);x+=kw+KEY_GAP;}}
    }
    private void renderFlash(){if(flashTimer<=0)return;flashTimer--;int fw=textRenderer.getWidth(flashMsg),fx=this.width/2-fw/2,fy=GRID_TOP-14;fill(fx-6,fy-2,fx+fw+6,fy+12,C_WHITE);textRenderer.draw(flashMsg,fx,fy+1,0xFF000000);}
    ...
    private void drawCentered(String text,int cx,int cy,int color){textRenderer.draw(text,cx-textRenderer.getWidth(text)/2,cy-4,color);}
