    ...
    private void renderKeyboard(MatrixStack matrices){
        ...
        for(int row=0;row<KB_ROWS.length;row++){
            ...
            for(String key:keys){
                ...
                textRenderer.draw(matrices,key,x+kw/2-textRenderer.getWidth(key)/2,y+KEY_H/2-4,C_WHITE);
                x+=kw+KEY_GAP;
            }
        }
    }
    private void renderFlash(MatrixStack m){
        if(flashTimer<=0)return;flashTimer--;
        int fw=textRenderer.getWidth(flashMsg),fx=this.width/2-fw/2,fy=GRID_TOP-14;
        fill(m,fx-6,fy-2,fx+fw+6,fy+12,C_WHITE);textRenderer.draw(m,flashMsg,fx,fy+1,0xFF000000);
    }
    private boolean isWide(String k){return k.equals("ENT")||k.equals("DEL");}
    private void drawRect(MatrixStack m,int x,int y,int w,int h,int t,int c){
        fill(m,x,y,x+w,y+t,c);fill(m,x,y+h-t,x+w,y+h,c);fill(m,x,y,x+t,y+h,c);fill(m,x+w-t,y,x+w,y+h,c);
    }
    private void drawCentered(MatrixStack m,String text,int cx,int cy,int color){
        textRenderer.draw(m,text,cx-textRenderer.getWidth(text)/2,cy-4,color);
    }
