...
public class WordleScreen extends Screen {
    ...
    private int panelWidth()  {
        return Math.max(10 * KEY_W + 9 * KEY_GAP + 16, 5 * TILE_SIZE + 4 * TILE_GAP + 20);
    }

    private int tileColor(TileState ts) {
        if (ts == null) return C_KEY_DEFAULT;
        return switch (ts) {
            case CORRECT -> C_CORRECT; case PRESENT -> C_PRESENT;
            case ABSENT  -> C_ABSENT;  default      -> C_TILE_EMPTY;
        };
    }

    private static int darken(int argb) {
        int a=(argb>>24)&0xFF, r=Math.max(0,((argb>>16)&0xFF)-28),
            g=Math.max(0,((argb>>8)&0xFF)-28), b=Math.max(0,(argb&0xFF)-28);
    ...
    private void playWinSound(int guesses) {
        var sound = guesses <= 2 ? SoundEvents.UI_TOAST_CHALLENGE_COMPLETE
            : guesses <= 4 ? SoundEvents.ENTITY_PLAYER_LEVELUP
    ...
    private void copyShare() {
        MinecraftClient.getInstance().keyboard.setClipboard(buildShareText()); flash("Copied!");
    }

    private void sendToChat() {
        MinecraftClient client = MinecraftClient.getInstance();
        if (client.player == null) { flash("Not in a world!"); return; }
        client.player.sendChatMessage(buildShareText().split("\n")[0]);
        flash("Sent to chat!");
    }
    ...
    private String buildShareText() {
        ...
        if (practiceMode) { sb.append("MineWordle Practice ").append(result).append("\n\n"); }
        else {
            long dayNum = ChronoUnit.DAYS.between(LocalDate.of(2021,6,20), LocalDate.now(ZoneId.systemDefault())) + 1;
        ...
        for (int row = 0; row < guesses; row++) {
            for (int col = 0; col < 5; col++) {
                sb.append(switch (eval[row][col]) {
                    case CORRECT -> "🟩"; case PRESENT -> "🟨"; default -> "⬜";
                });
            }
    ...
    public void render(MatrixStack matrices, int mx, int my, float delta) {
        ...
        int pw = panelWidth(), panelLeft = (this.width - pw) / 2;
        ...
        drawCenteredText(matrices, textRenderer, new LiteralText("MINEWORDLE"), this.width / 2, panelTop + 4, C_WHITE);
        fill(matrices, this.width / 2 - 80, panelTop + 14, this.width / 2 + 80, panelTop + 15, C_BORDER);
        if (practiceMode)
            drawCenteredText(matrices, textRenderer, new LiteralText("- PRACTICE -"), this.width / 2, panelTop + 17, C_PRACTICE);
        renderLoadingStatus(matrices); renderGrid(matrices); renderKeyboard(matrices);
        renderEndOverlay(matrices); renderFlash(matrices);
    ...
    private void renderEndOverlay(MatrixStack matrices) {
        ...
        String line1 = won ? switch (guesses) {
            case 1->"Genius!"; case 2->"Magnificent!"; case 3->"Impressive!";
            case 4->"Splendid!"; case 5->"Great!"; default->"Phew!";
        } : "Game over!";
        String line2 = "The word was: " + game.getSolution();
        String copyLabel = "[ COPY ]", chatLabel = "[ CHAT ]";
        int color1 = won ? C_GREEN : C_RED;
        int w1 = textRenderer.getWidth(line1), w2 = textRenderer.getWidth(line2);
        int wBtn = textRenderer.getWidth(copyLabel) + 20;
        int boxW = Math.max(Math.max(w1, w2), wBtn) + 28, boxH = 68;
        ...
        int btnPad = 12, btnW = boxW - btnPad * 2, btnH = 13;
        copyBtnW = btnW; copyBtnH = btnH; copyBtnX = boxX + btnPad; copyBtnY = boxY + 33;
        ...
        chatBtnW = btnW; chatBtnH = btnH; chatBtnX = boxX + btnPad; chatBtnY = copyBtnY + btnH + 3;
    ...
    private void renderGrid(MatrixStack matrices) {
        ...
        for (int row = 0; row < 6; row++) {
            for (int col = 0; col < 5; col++) {
                ...
                if (row < guessIdx) {
                    TileState ts = eval[row][col]; bg = tileColor(ts); border = bg;
                    if (guesses[row] != null && col < guesses[row].length())
                        letter = String.valueOf(guesses[row].charAt(col));
                } else if (row == guessIdx && playing && col < current.length()) {
                    letter = String.valueOf(current.charAt(col)); border = C_BORDER_ACTIVE;
                }
    ...
    private boolean isWide(String key) { return key.equals("ENT") || key.equals("DEL"); }

    private void drawRect(MatrixStack matrices, int x, int y, int w, int h, int t, int color) {
        fill(matrices,x,y,x+w,y+t,color); fill(matrices,x,y+h-t,x+w,y+h,color);
        fill(matrices,x,y,x+t,y+h,color); fill(matrices,x+w-t,y,x+w,y+h,color);
    }
